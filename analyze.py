...
class Access:
   def  __init__(self, access_times):
       self.sqr = access_times[0]<200
       self.mul = access_times[1]<200
       self.div = access_times[2]<200
...
d_p_after = d_p[max(d_p_indices)+1:]

print(spy_after)
print(f"length of spy after {len(spy_after)}")
print("------------------------------")
print(d_p_after)
...
spy_lcs_indices = pylcs.lcs_string_idx(d_p, spy_out)

print("-----LCS DATA------")
print(f"Len of slots {len(slots)}")
for i in spy_lcs_indices:
    if(i==-1):
        continue
    print(slots[i])
print()
